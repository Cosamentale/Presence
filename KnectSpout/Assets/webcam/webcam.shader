    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_target
            {
				...
				float3 tm1 = lerp(t1, lerp(t3, t2, r1), u1);
				float3 tm2 = lerp(t2, lerp(t1, t3, r1), u1);
				float3 tm3 = lerp(t3, lerp(t1, t2, r1), u1);
				float3 tm4 =  lerp(lerp(tm1, tm2, r2), tm3, r3);
				float3 tn1 =  lerp(lerp(lerp(t2, t3, u1), lerp(t3, t2, u1), r1), t1, u2);
				float3 tn2 = lerp(lerp(lerp(t1, t3, u1), lerp(t3, t1, u1), r1), t2, u2);
				float3 tn3 = lerp(lerp(lerp(t2, t1, u1), lerp(t2, t1, u1), r1), t3, u2);
				float3 tn4 = lerp(lerp(tn1, tn2, r2), tn3, r3);
				float3 to1 =  lerp( lerp(lerp(t2, t3, u2), lerp(t3, t2, u2), r1), t1, u1);
				...
				float pc4 = lerp(lerp(pc1, pc2, smoothstep(0.45, 0.55, no(_c1*8. + 95.17))), pc3, smoothstep(dt - 0.05*dt, dt + 0.05*dt, no(_c1*6. + 451.62)));
				float pc5 = pow(clamp(pc4,0.,1.), _float3);
				float n4 = smoothstep(0.6, 0.8, no(_c4*0.08+152.))*_step2invert;
